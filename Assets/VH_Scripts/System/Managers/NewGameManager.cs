...
namespace SmallTown {
public class NewGameManager : MonoBehaviour, IManager, IMenuGroup {
	...
	public void CheckGroups(CanvasGroup group, bool alsoCurrent = false) {
		for (int i = 0; i < canvasGroups.Count; i++) {
			if(alsoCurrent) {
				...
				canvasGroups[i].blocksRaycasts = group == canvasGroups[i] || _currentGroup == canvasGroups[i];
			}
			else {
				canvasGroups[i].blocksRaycasts = _currentGroup == canvasGroups[i];
		...
		// for (int i = 0; i < canvasGroups.Count; i++) {
		// 	if (group != null) {
		// 		Debug.LogWarning("Checking against " + group.name);
		// 	}
		// 	canvasGroups[i].gameObject.SetActive((group != null ? group : _currentGroup) == canvasGroups[i]);
		// }
	}
	...
	// ************************************************************************************************************ CUSTOM FUNCTIONS: CONTINUE

	public void ContinueGame() {
		Debug.LogWarning("Continue game pressed");
	}
}
}